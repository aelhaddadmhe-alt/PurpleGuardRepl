...
- **PurpleSOC Discovery Questionnaire (calculation rules)**: EDR row carries `per100:true`. Both `devLDS` and `devEPS` honor the flag — EDR LDS = `Math.ceil(cnt/100)` and EDR EPS = `Math.ceil(cnt/100) × epsTyp` (so 30 EPS *per LDS unit*, not per endpoint). 300 EDR endpoints → 3 LDS and 90 EPS; 301 endpoints → 4 LDS and 120 EPS. Workstations remain `per50:true` for LDS (per-50 ceiling) with EPS as `cnt × 3` (per-individual workstation, not per LDS). The `eps` value used for storage falls back to `grand × 40` when no device EPS or manual override is set. `SPECS` has no `epsMax` field; the server-spec card picks tier purely by total LDS via `getTier(SPECS, grand)`.
