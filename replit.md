...
- **PurpleSOC Discovery Questionnaire (calculation rules)**: EDR row carries `per100:true` so `devLDS` divides endpoint count by 100 (1 LDS per 100 endpoints, ceiling). `devEPS` ignores `per100` and counts every endpoint as `cnt × epsTyp` — so 300 EDR endpoints = 3 LDS but 9,000 EPS. Workstations remain `per50:true` for LDS (per-50 ceiling) with EPS as `cnt × 3`. The `eps` value used for storage falls back to `grand × 40` when no device EPS or manual override is set. `SPECS` has no `epsMax` field; the server-spec card picks tier purely by total LDS via `getTier(SPECS, grand)`. This matches the original standalone HTML, with the single intentional deviation of EDR LDS being divided by 100.
